...
export default {
	async fill_field(_fieldname, value, keyword) {
		...
		if (fieldtype == "Link" || fieldtype == "Dynamic Link") {
			...
			await this.pause(1);
			// click link option
			await this.click_element(`.dataent-control[data-fieldname="${fieldname}"] .link-field li`, value);
		}
		else if (fieldtype == "Select") {
			await t.click(input);
			await this.pause(0.6);

			const sel = `select[data-fieldname="${fieldname}"]`;
			await ClientFunction(() => {
				// dataent.show_alert('Submitted');
	...
	async scroll_to_field(fieldname) {
		...
		if (fieldname) {
			this.scroll_to_element(`.dataent-control[data-fieldname="${fieldname}"]`)
		}
	...
	async get_field(fieldname) {
		...
		var field = await Selector(() => {
			...
			if ($('body').hasClass('modal-open')) {
				...
				var field = cur_frm.cur_grid.get_field(fieldname);
				return return_field(field);
			} else if(cur_frm) {
				// Priority 3: Form
